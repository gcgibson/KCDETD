...
rRevisedILI <- function(
  ...
  if(region %in% c('nat', paste0('hhs', 1:10))) {
    ...
    'pa', 'ri', 'sc', 'sd', 'tn', 'tx', 'ut', 'vt', 'va', 'wa', 'wv', 'wi', 'wy',
    'as', 'mp', 'dc', 'gu', 'pr', 'vi', 'ord', 'lax', 'jfk')) {
    flu_data_with_backfill <- readRDS("../flu_data_with_backfill.rds")
  } else {
  ...
  for (i in 1:n) {
    ...
    if (epiweek_idx <= 20 & epiweek_idx >= 10 ){
      sampled_epiweek <- paste0("2018",epiweek_idx)
    } else if(epiweek_idx < 10){
      sampled_epiweek <- paste0("20180",epiweek_idx)
    } else{
      sampled_epiweek <- paste0("2017",epiweek_idx)
    }
    ...
    delta <- tail(avail$ili,time_in)-tail(fully_obs$ili,time_in)
